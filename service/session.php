 ...
 */
class SessionManagement extends PDCommonClass {
    ...
    public function getRequestToken() {
        if ( (!empty($this->requestToken)) && (!empty($this->requestTokenSecret)) ) {
    ...
    public function getVerificationCode() {
        if ( (!empty($this->requestToken)) && (!empty($this->verificationCode)) ) {
    ...
    public function generateRequestToken() {
        ...
        $sth = $this->dbh->prepare("INSERT INTO requesttokens (consumer_key, request_token, request_token_secret) VALUES (?,?,?)");
        $res = $sth->execute(array($this->consumerKey,$this->requestToken, $this->requestTokenSecret));
        if( PEAR::isError($res) ){
    ...
    public function generateAccessToken() {
        ...
        // we want to store the two items into the data base.
        $sth = $this->dbh->prepare("INSERT INTO accesstokens (consumer_key, access_token, access_token_secret, user_id) VALUES (?,?,?,?)");
        $res = $sth->execute(array($this->consumerKey,$this->accessToken, $this->accessTokenSecret, $this->userID));
        if( PEAR::isError($res) ){
            $this->log("database error " . $res->getMessage());
        }
        else {
    ...
    public function generateVerificationCode() {
        if ($this->requestTokenID > 0 && $this->userID > 0) {
            ...
            $res = $sth->execute(array($this->userID, $this->verificationCode, $this->requestTokenID));
            if (PEAR::isError($res)) {
    ...
    public function timestampNonceChecker($provider) {
        ...
        if ( !(empty($this->accessToken) && empty($this->requestToken))) {
            ...
            if ($this->accessTokenID > 0) {
                ...
                $res = $sth->execute(array($this->consumerID, $this->accessTokenID, $nonce));
            }
            else if ($this->requestTokenID) {
                $sth = $this->dbh->prepare("SELECT nonce FROM nonce_list WHERE consumer_id =? AND request_id = ? AND nonce =?");
                $res = $sth->execute(array($this->consumerID, $this->requestTokenID, $nonce));
            }
        ...
        if ( !(empty($this->accessToken) && empty($this->requestToken))) {
            ...
            $res = $sth->execute(array($nonce, $this->consumerID, $this->accessTokenID, $this->requestTokenID));
            if ( PEAR::isError($res)) {
                $this->log('DB ERROR: '. $res->getMessage());
            }
            $sth->free();
    ...
    public function lookupConsumer($provider) {
        ...
        if ($this->consumerKey != $provider->consumer_key || empty($this->consumerSecret) ) {
    ...
    public function lookupRequestTokenVerification($provider) {
        ...
        if ($this->requestToken != $provider->token ||
            empty($this->requestTokenSecret)) {
            $this->oauthState = $this->initRequestTokenVerification($provider->token, $provider->verifier);
        }
    ...
    protected function initRequestToken($token) {
        ...
        if (!empty($token)) {
            // this function only returns true if there is a secret for the token
            $sth = $this->dbh->prepare("SELECT id, request_token_secret, verification, UNIX_TIMESTAMP(created_date), authorised_user_id  FROM requesttokens WHERE consumer_key = ? AND request_token = ?");
    ...
    protected function initRequestTokenVerification($token, $verifier) {
        ...
        // this function only returns true if there is a secret for the token
        $sth = $this->dbh->prepare("SELECT id, request_token_secret, verification, UNIX_TIMESTAMP(created_date), authorised_user_id FROM requesttokens WHERE consumer_key = ? AND request_token = ? AND verification = ?");
        $res = $sth->execute(array($this->consumerKey, $token, $verifier));
        if (!PEAR::isError($res)) {
            if ($res->numRows() > 0) {
                $row = $res->fetchRow();
    ...
    protected function initAccessToken($token) { 
        ...
        $sth = $this->dbh->prepare("SELECT id, access_token_secret, user_id, UNIX_TIMESTAMP(create_time) FROM accesstokens WHERE consumer_key = ? AND access_token = ?");
        $res = $sth->execute(array($this->consumerKey, $token));
        if (!PEAR::isError($res)) {
            if ($res->numRows() > 0) {
                $row = $res->fetchRow();
    ...
    protected function clearRequestTokenAndNonces($id) {
        ...
        if (!PEAR::isError($res)) {
            ...
            $res = $sth->execute(array($this->consumerID, $id));
        }
        $sth->free();
    }
    ...
    protected function clearAccessTokenAndNonces($id) {
        ...
        $res = $sth->execute(array($this->consumerKey, $this->accessToken));
        if (!PEAR::isError($res)) {
            $sth->free();
