 ...
 * management.
 *
 * It implements all OAuth steps as well as the invalidation of accesstokens and the management of
...
*/
    ...
    protected function update_userprofile() {
        ...
        try {
            ...
            $this->log( 'did NOT encode user data' );
            $this->bad_request();
            return;
        }
        ...
        if ( !PEAR::isError( $res ) ) {
            ...
            if ( !PEAR::isError( $res2 ) ) {
                ...
                $sqlstring2 = "UPDATE users SET title = ?, name = ? WHERE id = ?";
                $sth2->free();
                $sth2 = $this->dbh->prepare($sqlstring2);

                //DISALLOW EMAIL CHANGE
                // $sth2->execute(array($tmp['title'],$tmp['name'], $tmp['email'], $uid));
                $sth2->execute( array( $tmp['title'] , $tmp['name'] , $uid ) );
                $sth2->free();
            } else {
    ...
    protected function register_user(){
        ...
        if ( count( $empty_fields ) > 0 ) {
            ...
            $jsonErrorObject = array( "empty"=>$empty_fields );
        ...
        if ( !$resp->is_valid ) {
            ...
            $this->log( "captcha entered incorrectly" );
            $this->bad_request();
            return;
        }
        ...
        if ( $resCheck->numRows() == 1 ) {
            ...
            $this->log( "the email already exists in the database" );
            $this->forbidden();
            return;
        }
    ...
    protected function update_password() {
        ...
        // POST BASE_URI
        $this->log( 'update password called' );
        ...
        try {
            ...
            $tmp = json_decode( $data , true );
            $this->log( 'decoded password data succesfully' );
        } catch ( Exception $e ) {

            $this->log(' could NOT decode password data' );
            $this->bad_request();
            return;
        }
        ...
        // -- start
        // what is the purpose of this whole section?
        $data = json_encode( $tmp );
        $uid2=$data['user_id'];
