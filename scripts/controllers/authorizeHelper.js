...
function AuthorizationController() {
    ...
    self.getActiveDossier = function() {
        ...
        var dossierID = mUser.getActiveDossier();
        if ( !dossierID ) {

            dossierID = mDossiers.getDefaultDossierId();
        }
        ISNLogger.log( 'active dossier is : ' + dossierID );
        return dossierID;
    };
    ...
    $(document).bind('BOOKMARKSTORED', function() {
        ...
        window.parent.postMessage( JSON.stringify({'bookmarkok' : 1}), targetHost );
    });
    ...
    function checkBookmark(){
        ...
        if ( self.activeItemID && bookmarks.hasItem( self.activeItemID ) ) {
            ...
            window.parent.postMessage( JSON.stringify({'bookmarkok': 1}), self.targetHost );
        }
    }
}
