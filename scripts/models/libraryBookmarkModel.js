...
LibraryBookmarkModel.prototype.addItem = function( dossierID ) {
    ...
    if ( dossierID && id ) {
        ...
        //visually indicate progress and disable button click
        $('#item' + dossierID).parent().removeClass('clickable').css({'cursor':'wait'});

        //also disable the children
        $('#item' + dossierID + ', #item' + dossierID + ' *').removeClass('clickable').css({'cursor':'wait'});
    ...
    function complete(){
        ...
        // on completion of our ajax whatever the result
        //return clickability to the bookmark and updating the cursor style
        ISNLogger.log( self.library_item_id );
        self.getUserBookmarkedDossiers();
    }
